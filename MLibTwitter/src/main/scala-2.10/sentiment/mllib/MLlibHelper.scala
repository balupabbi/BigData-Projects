...
object MLlibHelper {
  ...
  def computeSentiment(text: String, stopWordsList: Broadcast[List[String]], model: NaiveBayesModel): Int = {
    ...
    val tweetInWords: Seq[String] = cleanTweets(text, stopWordsList.value)
    val polarity = model.predict(MLlibHelper.transformFeatures(tweetInWords))
   ...
   */
  def cleanTweets(tweetText: String, stopWordsList: List[String]): Seq[String] = {
