...
object SparkCoreNLPTweetSentimentAnalyzer extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    rawTweets.foreachRDD { rdd =>
      if (rdd != null && !rdd.isEmpty() && !rdd.partitions.isEmpty) {
        saveRawTweetsInJSONFormat(rdd, PropertiesLoader.TWEETS_RAW_PATH)
      }
    }
    ...
    classifiedTweets.foreachRDD { rdd =>
      if (rdd != null && !rdd.isEmpty() && !rdd.partitions.isEmpty) {
        //val DELIMITER = "|"
        saveClassifiedTweets(PropertiesLoader.TWEETS_CLASSIFIED_PATH, rdd)
    ...
    ssc.start()
    ssc.awaitTerminationOrTimeout(PropertiesLoader.OTAL_RUN_TIME_IN_MINUTES * 60 * 1000) //auto-kill after processing tweets for 'n' mins.
  }
  ...
  def createSparkStreamingContext(): StreamingContext = {
    ...
    val ssc = new StreamingContext(conf, Durations.seconds(PropertiesLoader.MICRO_BATCH_TIME_IN_SECONDS)) // 'n' Seconds batch interval
    ssc
  }
